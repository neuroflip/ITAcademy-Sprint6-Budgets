    ...
    return (<div className={ `card__container container ${ budget.isChecked ? 'card__container--selected' : ''}` }>
        ...
        <div className="col-start-1 col-end-6 row-span-1 justify-self-end mt-5">
            { budget && budget.extras && budget.isChecked && <>
                { budget.extras && <BudgetServiceExtras text="Pages number" infoMessage="Add the pages number that your peoject will have. It will add 30€ per page." id='0' value={ budget.extras && budget.extras[0].value } onChange={ onChangeExtras } /> }
                { budget.extras && <BudgetServiceExtras text="Languages number" infoMessage="Add the languages number that your project will have. It will add 30€ per language." id='1' value={ budget.extras[1].value } onChange={ onChangeExtras } /> }</>
            }
        </div>
    </div>)
}
