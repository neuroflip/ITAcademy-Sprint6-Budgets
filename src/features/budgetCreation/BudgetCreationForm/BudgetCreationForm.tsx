...
const BudgetCreationForm = ({ onBudgetCreation }: BudgetCreationFormProps) => {
  ...
  return <div className="my-20 container text-left dropShadow p-10 rounded-xl">
      ...
      <form className="budgetForm my-5 grid grid-cols-1 sm:grid-cols-4 gap-3 items-center" action={ submitAction } noValidate>
        ...
        <label className="row-start-2 sm:row-start-1">
          Telephone:
          <input value = { telephone } onChange={ validateTelephone } className="w-full sm:w-40 border border-gray-500 rounded mr-3 p-3" minLength={1} maxLength={15} type="tel" placeholder="Telephone Number..." id="telephone" name="telephone" />
        </label>
        <label className="row-start-3 sm:row-start-1">
          Email:
