...
const BudgetServiceCard = ({ budget, onChangeBudget, discount }: BudgetServiceCardProps) => {
    ...
    return (<div className={ `card__container container ${ budget.isChecked ? 'card__container--selected' : ''}` }>
        ...
        <label className="text-right col-start-4 col-end-6 row-start-1 sm:row-start-2 justify-self-start ml-1 text-sm ">
            { `Add ${budget.title}` }
            <input id={`apply${budget.id}`} onChange={ onClickCheckBox } type="checkbox" className="ml-3 justify-self-end w-4 h-4 border rounded" checked={ budget.isChecked } />
        </label>
